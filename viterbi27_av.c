...
int init_viterbi27_av(void *p,int starting_state){
  ...
  if(p == NULL)
    return -1;
  for(i=0;i<4;i++)
    vp->metrics1.v[i] = (vector unsigned char){63, 63, 63, 63, 63, 63, 63, 63, 63, 63, 63, 63, 63, 63, 63, 63};
...
int update_viterbi27_blk_av(void *p,unsigned char *syms,int nbits){
  ...
  while(nbits--){
    ...
    metric = vec_sr(metric,(vector unsigned char){3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3});
    m_metric = vec_sub((vector unsigned char){31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31},metric);

...
int main(int argc,char *argv[]){
  ...
  if((vp = create_viterbi47(framebits)) == NULL){
    printf("create_viterbi47 failed\n");
    exit(1);
  }
  if(ebn0 != -100){
	...
	symbols[3*i+2] = addnoise(parity(sr & V47POLYC),gain,Gain,127.5,255);
      ...
      /* Decode block */
      update_viterbi47_blk(vp,symbols,framebits+8);
      
      /* Do Viterbi chainback */
      chainback_viterbi47(vp,data,framebits,0);
      errcnt = 0;
      for(i=0;i<framebits/8;i++){
    ...
    for(tr=0;tr < trials;tr++){
      ...
      /* Decode block */
      update_viterbi47_blk(vp,symbols,framebits);
